...
def findBestHorizontally():
    best = 0
    current = 1
    for xIndex in range(0, 20):
        for yIndex in range(0, 17):
            for indexOffset in range(0, 4):
                current *= grid[xIndex][yIndex + indexOffset]
            best = max(best, current)
            current = 1
    return best


def findBestVertically():
    best = 0
    current = 1
    for yIndex in range(0, 20):
        for xIndex in range(0, 17):
            for xIndexOffset in range(0, 4):
                current *= grid[xIndex + xIndexOffset][yIndex]
            best = max(best, current)
            current = 1
    return best


def findBestDiagonally():
    best = 0
    current = 1
    ...
    for yIndex in range(0, 17):
        for xIndex in range(0, 17):
            for indexOffset in range(0, 4):
                current *= grid[xIndex + indexOffset][yIndex + indexOffset]
            best = max(best, current)
            current = 1

    # Possibility 2: //
    for yIndex in range(3, 20):
        for xIndex in range(0, 17):
            for indexOffset in range(0, 4):
                current *= grid[xIndex + indexOffset][yIndex - indexOffset]
            best = max(best, current)
...
def solve():
    bestH = findBestHorizontally()
    bestV = findBestVertically()
    bestD = findBestDiagonally()

    print("Best horizontally:   " + str(bestH))
    print("Best vertically:     " + str(bestV))
    print("Best diagonally:     " + str(bestD))

    best = max(bestH, bestD, bestV)

    print("Best overall:        " + str(best))
